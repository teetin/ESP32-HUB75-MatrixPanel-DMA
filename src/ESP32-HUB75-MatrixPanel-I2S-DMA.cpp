 ...
 */
void MatrixPanel_I2S_DMA::drawLVGLBitmap(int16_t x, int16_t y, int16_t w, int16_t h, const void* data) {
    ...
    if (color_depth == 16) {
        ...
        for (int16_t j = 0; j < h; j++) {
            for (int16_t i = 0; i < w; i++) {
                uint32_t c = *colors++;
                drawPixelRGB888(x + i, y + j, (uint8_t)(c >> 16), (uint8_t)(c >> 8), (uint8_t)c);
            }
        }
    }
}
